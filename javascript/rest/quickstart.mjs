...
async function cmdRoundtrip(config, args) {
  ...
  const { order } = await request(config, "POST", "/api/v1/orders", {
    ...
    order_type: "limit",
    action: "buy",
    // Both are STRINGS. Sending the number 51 as a price is a 400: an integer
    // used to mean 51 cents, and reading it as $51.00 would be a 100x
    // overprice, so the server rejects it rather than guessing. quantity is a
    // string for a different reason - a float arrives as an IEEE-754 double, so
    // 2.675 would rest on the book as 2.67499999... A number is a 400 either
    // way; the decimal string is the only spelling.
